...
for breaker_xml_BE in root_SSH_BE.iter(cim+"Breaker"):
    for i in range(len(breaker_list_BE)):
        if breaker_list_BE[i].rdf_ID == breaker_xml_BE.get(rdf+"about").lstrip("#"):
            breaker_list_BE[i].state=breaker_xml_BE.find(cim+"Switch.open").text
...
# print("Length:",len(power_transformer_end_transformer_winding_list))
# Missing nodes
count=0
